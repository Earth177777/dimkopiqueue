...
mongoose.connect(MONGODB_URI)
    .then(async () => {
        ...
        // and fix any hardcoded localhost/webtinous domains that were accidentally saved
        const menuItemsToMigrate = await MenuItem.find();
        for (let item of menuItemsToMigrate) {
            let changed = false;
            let currentImage = item.image;

            if (currentImage && currentImage.includes('/uploads/') && !currentImage.includes('/api/uploads/')) {
                currentImage = currentImage.replace('/uploads/', '/api/uploads/');
                changed = true;
            }

            if (currentImage) {
                // Strip out full domain urls (generated by the buggy frontend or older test code)
                // E.g. http://localhost:5001/api/uploads/img-123.png -> /api/uploads/img-123.png
                // E.g. https://r1booking.webtinous.com/api/uploads/img-123.png -> /api/uploads/img-123.png
                // E.g. http://r1booking.webtinous.com:5001/api/uploads/img-123.png -> /api/uploads/img-123.png
                const domainSuffixes = [
                    'http://localhost:5001',
                    'https://r1booking.webtinous.com',
                    'http://r1booking.webtinous.com:5001',
                    'http://103.59.95.58:5001'
                ];

                for (const suffix of domainSuffixes) {
                    if (currentImage.startsWith(suffix)) {
                        currentImage = currentImage.replace(suffix, '');
                        changed = true;
                    }
                }
            }
            ...
            if (changed) {
                ...
                await item.save();
                console.log(`Migrated image URL for item ${item.name}`);
            }
        }
    })
